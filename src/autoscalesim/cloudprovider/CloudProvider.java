...
package autoscalesim.cloudprovider;

import org.cloudbus.cloudsim.Storage;
import org.cloudbus.cloudsim.ResCloudlet;
import static autoscalesim.applicationprovider.ApplicationProvider.IsFutureCloudlet;
...
import static autoscalesim.enduser.EndUserEmulator.SIMULATION_LIMIT;
 ...
 */
public class CloudProvider extends SimEntity {
	...
	public CloudProvider(
		...
		getCharacteristics().setId(super.getId());
                ...
                //my code
                this.cloudletTimeout = cloudletTimeout;
	}
...
//                                checkCloudletTimeout();
				break;
                        case AutoScaleSimTags.VMS_SYNCHRONIZATION:
                                updateCloudletProcessing();
				checkCloudletCompletionPeriodically(ev);
//                                checkCloudletTimeout();
				break;
                        case AutoScaleSimTags.TIMEOUT_CHECKER:
                                checkCloudletTimeout();
                                break;
	...
	protected void processVmCreate(SimEvent ev, boolean ack) {
		Vm vm = (Vm) ev.getData();
                // entekhab hast, ba olaviate host ba freepes bishtar.
//                agar yek host 1 pe ba 1000 mips darad emkan in hast ke 2 vm ke majmoe mips kamtar
//                az 1000 darand hamzaman roye an gharar begirand va bedone takhir zaman bandi shavand.
...
//                        int vmId = vm.getId();
	...
	protected void processCloudletMove(int[] receivedData, int type) {
		...
		if (cl == null) {// cloudlet doesn't exist
			...
			if (cl.getCloudletStatus() == Cloudlet.SUCCESS) {// if yes, send it back to user
				...
				sendNow(cl.getUserId(), CloudSimTags.CLOUDLET_COMPLETE, cl);
			}
        ...
        //it just works in time shared scheduling
        protected void checkCloudletTimeout(){
            double maxAllowedSubmissionTime = CloudSim.clock() - cloudletTimeout;
            ArrayList cloudletToFailure = new ArrayList();
            if (CloudSim.clock() > 1 ) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    for (Vm vm : host.getVmList()) {
                        // for each cloudlet in the VM Scheduler
                        //executed cloudlets
                        for(ResCloudlet resCloudlet : ((CloudletSchedulerTimeShared)vm.getCloudletScheduler()).getCloudletExecList()){
                            if (resCloudlet.getCloudlet().getFirstSubmissionTime() <= maxAllowedSubmissionTime){
                                // should be failed
                                int cloudletId = resCloudlet.getCloudletId();
                                int userId = resCloudlet.getCloudlet().getUserId();
                                int vmId = vm.getId();
                                
                                int[] cloudletInfo = new int[3];
                                cloudletInfo[0] = cloudletId;
                                cloudletInfo[1] = userId;
                                cloudletInfo[2] = vmId;
                                cloudletToFailure.add(cloudletInfo);
                            }
                        }
                        //paused cloudlets
                        for(ResCloudlet resCloudlet : ((CloudletSchedulerTimeShared)vm.getCloudletScheduler()).getCloudletPausedList()){
                            if (resCloudlet.getCloudlet().getFirstSubmissionTime() < maxAllowedSubmissionTime){
                ...
                for(int i = 0; i < cloudletToFailure.size(); i++){
                    int[] cloudletInfo = (int[])cloudletToFailure.get(i);
                    int cloudletId = cloudletInfo[0];
                    int userId = cloudletInfo[1];
                    int vmId = cloudletInfo[2];
                    
                    processCloudletFailure(cloudletId, userId, vmId);
                    
                }
            } 
            
            // recall yourself
            if (IsFutureCloudlet || (CloudSim.clock() < (SIMULATION_LIMIT * 60 * 1.1))){
                send(getId(), 1, AutoScaleSimTags.TIMEOUT_CHECKER);
            }
	...
	protected void processCloudletCancel(int cloudletId, int userId, int vmId) {
            //khodash az beyne exec va paused ha migardad peydash  mikonad va cancel mikonad
		Cloudlet cl = getVmAllocationPolicy().getHost(vmId, userId).getVm(vmId,userId)
				.getCloudletScheduler().cloudletCancel(cloudletId);
		sendNow(userId, CloudSimTags.CLOUDLET_CANCEL, cl);
	}

        protected void processCloudletFailure(int cloudletId, int userId, int vmId) {
            //khodash az beyne exec va paused ha migardad peydash  mikonad va fail mikonad
		Cloudlet cl = getVmAllocationPolicy().getHost(vmId, userId).getVm(vmId,userId)
				.getCloudletScheduler().cloudletCancel(cloudletId);
		sendNow(userId, AutoScaleSimTags.CLOUDLET_FAIL, cl);
	}
	/**
	...
	protected void checkCloudletCompletion() {
        ...
        protected void checkCloudletCompletionPeriodically(SimEvent ev) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (Vm vm : host.getVmList()) {
                ...
                // Send Answer
                sendNow(ev.getSource() , AutoScaleSimTags.VMS_SYNCHRONIZATION);
	}
	/**
...
//                else
//                    scheduleFirstNow(gisID, CloudSimTags.REGISTER_RESOURCE, getId());
